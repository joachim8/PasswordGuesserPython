...
from DatesWork import WorkOnDates
...
class Checker:
    ...
    def __init__(self, personalInfo:InfoPersonnel, optionWord:OptionWord, optionDate:OptionDate, optionAllChar:AllSpecialCharacters):
        ...
        self.optionAllChar = optionAllChar
        ##Composition
        self.optionCharCommun = CommonSpecialCharacters()


    # Return all the posibilities of each string équipe (list of lists)
    def getPossibleStrings(self):
        result = []
        for word in self.personalInfo.stringData:
            ...
            if self.optionWord.CharaSpecCommun:
                # Générer les caractères spéciaux communs et les ajouter aux options
                options.extend(self.generate_common_special_characters())
            if self.optionWord.CharaSpec:
                # Générer tous les caractères spéciaux et les ajouter aux options
                options.extend(self.generate_all_special_characters())
        return result
   ...
   # Méthode pour générer les caractères spéciaux communs
    def generate_common_special_characters(self, length=1):
        return self.optionCharCommun.generate(length)

    # Méthode pour générer tous les caractères spéciaux
    def generate_all_special_characters(self, length=1):
        return self.optionAllChar.generate(length)
